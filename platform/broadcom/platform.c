...
void platform_mld_update(wifi_vap_info_t *vap)
{
	...
	wifi_hal_error_print("### %s: %s radio=%d vap_index=%d mld: enable=%d unit=%d linkid=%d apply=%d ###\n",
		__func__, vap->vap_name, vap->radio_index, vap->vap_index,
		mld_cmn->mld_enable, mld_cmn->mld_id,
                mld_cmn->mld_link_id, mld_cmn->mld_apply);
	if (mld_cmn->mld_enable && mld_cmn->mld_id < MAX_MLD_UNITS) {
		...
		if (vapidx != vap->vap_index) {
			wifi_hal_error_print("### %s: mld%d[%d] vap_index changes from %d to %d ###\n",
				__func__, mld_unit, vap->radio_index, vapidx, vap->vap_index);
	...
	if (_platform_init_done != FALSE && mld_cmn->mld_enable && mld_unit >= 0) {
		...
		for (i = 0; i < MAX_MLO_RADIOS; i++) {
			...
			wifi_hal_error_print("### %s: %s calling platform_bss_up(%d, %d) ###\n",
				 __func__, interface_name, vapidx, TRUE);
