...
struct hostapd_mld *get_mlo_mld(unsigned char mld_unit, char *mac)
{
    ...
    if (g_mlo_mld[mld_unit].name[0] == '\0') {
        ...
        snprintf(g_mlo_mld[mld_unit].name, sizeof(g_mlo_mld[mld_unit].name),
            "mld_unit_%u", mld_unit);
    }
...
struct hostapd_mld *get_slo_mld(wifi_vap_index_t vap_index, char *mac)
{
    ...
    if (g_slo_mld[vap_index].name[0] == '\0') {
        ...
        snprintf(g_slo_mld[vap_index].name, sizeof(g_slo_mld[vap_index].name),
            "slo_mld_id_%u", vap_index);
    }
